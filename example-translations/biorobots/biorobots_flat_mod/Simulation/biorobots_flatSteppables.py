...
chemotaxis_lambda = 1000

focal_length = 3

focal_lambda = chemotaxis_lambda/10_000
...
class ConstraintsSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        if pcp_imp:
            ...
            phenotype = pcp.phenotypes.get_phenotype_by_name('Simple Live')
            ...
            phenotype = pcp.phenotypes.get_phenotype_by_name('Standard apoptosis model')
            self.phenotypes['default']['Standard apoptosis model'] = phenotype(dt=dt,
                                                                               ...
                                                                               target_cytoplasm_to_nuclear_ratio=[
                                                                                   None],
                                                                               fluid_change_rate=[0.05])
            phenotype = pcp.phenotypes.get_phenotype_by_name('Standard necrosis model')
        ...
        while len(self.cell_list_by_type(self.CARGO_CELL)) < cluster_size * number_cargo_clusters:
            ...
            while not space:
                print(attempts)
                attempts += 1
                x = np.random.randint(9, self.dim.x - 18)
                y = np.random.randint(9, self.dim.y - 18)

                space = not bool(self.cell_field[x, y, 0])
                ...
                if space:
                    for nx, ny in neighbors:
                        ...
                        for sx, sy in shifts:
                            if not space:
                                break
                            space = space and not bool(self.cell_field[x + nx + sx, y + ny + sy, 0])

            for sx, sy in shifts:
                cell = self.new_cell(self.CARGO_CELL)
                self.cell_field[x + sx:x + sx + 3, y + sy:y + sy + 3, 0] = cell

        while len(self.cell_list_by_type(self.DIRECTOR_CELL)) < number_directors:
            space = False
            print("cells", len(self.cell_list_by_type(self.DIRECTOR_CELL)))
            attempts = 0
            while not space:
                print(attempts)
                attempts += 1
                x = np.random.randint(9, self.dim.x - 9)
                y = np.random.randint(9, self.dim.y - 9)
                space = not bool(self.cell_field[x, y, 0])
                ...
                if space:
                    for nx, ny in neighbors:
                        if not space:
                            break
                        space = space and not bool(self.cell_field[x + nx, y + ny, 0])
            cell = self.new_cell(self.DIRECTOR_CELL)
            self.cell_field[x:x + 3, y:y + 3, 0] = cell

        while len(self.cell_list_by_type(self.WORKER_CELL)) < number_workers:
            space = False
            print("cells", len(self.cell_list_by_type(self.WORKER_CELL)))
            attempts = 0
            while not space:
                print(attempts)
                attempts += 1
                ...
                if space:
                    for nx, ny in neighbors:
                        if not space:
                            break
                        space = space and not bool(self.cell_field[x + nx, y + ny, 0])
            cell = self.new_cell(self.WORKER_CELL)
            self.cell_field[x:x + 3, y:y + 3, 0] = cell
        ...
        for cell in self.cell_list_by_type(self.WORKER_CELL):
            cd = self.chemotaxisPlugin.addChemotaxisData(cell, "cargo_signal")
            cd.setLambda(chemotaxis_lambda)
            cd = self.chemotaxisPlugin.addChemotaxisData(cell, "director_signal")
            cd.setLambda(0)
            cell.dict["seeking"] = True
            cell.dict['volume'] = {
                'volume (micron^3)': 2494.0,
                'volume (pixels)': 8.105500000000003}
            cell.targetVolume = 12
        ...
        for field_name, secretor in self.secretors.items():
            for cell in self.cell_list_by_type(self.DEFAULT):
                if field_name in cell.dict.keys():
                    ...
                    seen = secretor.amountSeenByCell(cell) / cell.volume
            ...
            for cell in self.cell_list_by_type(self.CARGO_CELL):
                if field_name in cell.dict.keys():
                    ...
                    if data['uptake_rate']:
                        secretor.uptakeInsideCell(cell, 1e10, data['uptake_rate'])
            for cell in self.cell_list_by_type(self.WORKER_CELL):
                # set_focal_point_plasticity_parameters
...
class PhenotypeSteppable(MitosisSteppableBase):
    ...
    def start(self):
        ...
        self.mcs_to_time = float(self.get_xml_element(
            'mcs_to_time').cdata)  # MCS/[unit], see xml for units
        self.fields = ['director_signal', 'cargo_signal']
        pass
    ...
    def step(self, mcs):
        ...
        cells_to_divide = []
        for cell in self.cell_list_by_type(self.WORKER_CELL):
            if cell.dict["seeking"]:
                for neighbor, common_area in self.get_cell_neighbor_data_list(cell):
                    if neighbor and neighbor.type == self.CARGO_CELL and not len(self.get_fpp_links_by_cell(neighbor)) \
                            and not neighbor.dict["delivered"]:
                        cell.dict["seeking"] = False

                        cd = self.chemotaxisPlugin.getChemotaxisData(cell, "cargo_signal")
                        cd.setLambda(0)
                        cd = self.chemotaxisPlugin.getChemotaxisData(cell, "director_signal")
                        cd.setLambda(chemotaxis_lambda)

                        self.focalPointPlasticityPlugin.createFocalPointPlasticityLink(cell, neighbor, focal_lambda,
                                                                                       1, 10)
                        # cd = self.chemotaxisPlugin.addChemotaxisData(neighbor, "director_signal")
                        # cd.setLambda(chemotaxis_lambda)

            else:
                if not len(self.get_fpp_links_by_cell(cell)):
                    cell.dict["seeking"] = True
                    cd = self.chemotaxisPlugin.getChemotaxisData(cell, "cargo_signal")
                    cd.setLambda(chemotaxis_lambda)
                    cd = self.chemotaxisPlugin.getChemotaxisData(cell, "director_signal")
                ...
                for neighbor, common_area in self.get_cell_neighbor_data_list(cell):
                    if neighbor and neighbor.type == self.DIRECTOR_CELL:# or \
                            # (neighbor and neighbor.type == self.CARGO_CELL and neighbor.dict["delivered"]):
                        cell.dict["seeking"] = True
                        ...
                        for linked in self.get_fpp_linked_cells(cell):
                            linked.dict["delivered"] = True
                            linked.lambdaVecX = 0
                            linked.lambdaVecY = 0

                            self.remove_all_cell_fpp_links(linked)

                            self.new_fpp_link(linked, neighbor, 100*focal_lambda, 1, 20)

                            for field_name in self.fields:
                                if field_name in linked.dict.keys():
                                    linked.dict[field_name]['secretion_rate_MCS'] = 0
                                    linked.dict[field_name]['net_export_MCS'] = 0
                            # net_secretion = max(
                            #     0, data['secretion_rate_MCS'] * (data['secretion_target'] - seen)) + data[
                            #                     'net_export_MCS']

                        break
        for cell in self.cell_list_by_type(self.CARGO_CELL):
            if len(self.get_fpp_links_by_cell(cell)):
                for linked in self.get_fpp_linked_cells(cell):
                    dx = linked.xCOM - cell.xCOM
                    dy = linked.yCOM - cell.yCOM
                    angle = np.arctan2(dy, dx)

                    cell.lambdaVecX = - np.cos(angle) * 50
                    cell.lambdaVecY = - np.sin(angle) * 50
            else:
                cell.lambdaVecX = 0
                cell.lambdaVecY = 0
            if cell.dict["delivered"]:
                for neighbor, common_area in self.get_cell_neighbor_data_list(cell):
                    if neighbor and neighbor.type == self.DIRECTOR_CELL:
                        cell.type = self.DIRECTOR_CELL
        if pcp_imp:
            pass
            for cell in self.cell_list_by_type(self.DEFAULT):
